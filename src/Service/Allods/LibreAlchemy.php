...
use function App\Foundation\Helpers\{ app, container, config, name_format };

final class LibreAlchemy
{
	private bool $exception = true;//false;
    ...
    // del
	public function enableException( bool $bool = true ): void
	{
		$this -> exception = $bool;
	}
	...
	{
		container( \Arduino\Mouse :: class ) -> send ( 'p' );
	...
	{
		$bool = file_exists ( $this -> modsPath );
	...
	protected function readLastDataLibreAlchemy(): array
	{
		...
		//var_dump( 'preg_match_all', $matches, empty ( $matches ), '---------------' );
		...
		{
			throw new AlchemyException( 'Последние данные не прошли проверку актуальности: ' . json_encode ( compact ( 'time', 'action', 'data' ), 480 ) );
		}
	...
	public function checkUIAlchemy(): bool
	{
		...
		if ( ! $bool && $this -> exception )
		{
			throw new AlchemyException( 'Не распознан UI интерфейс. Перезапустите LibreAlchemy' );
		}
	...
	public function checkReady(): bool
	{
		...
		if ( ! $bool && $this -> exception )
		{
			throw new AlchemyException( 'Интерфейс не готов: {OnAlchemyReactionFinished:empty}' );
		}
	...
	public function checkIngredients( array &$ingredients ): void
	{
		...
		{
			// смысл переводить в заглавную букву строки ? изначально в бд уже есть
			//$normalizedComponents = array_map ( name_format :: class, $components );
			...
			foreach ( $this -> aspects AS $key => $aspectName )
			{
				...
				// 'Не найден один из компонентов %s: %s', 
				
				// проверяет в начале аспекты ингредиентов
				if ( $flag )
				{
	...
	// OnAlchemyItemPlaced:2
	// OnAlchemyCanceled:0
	// del
	public function countPossiblePotions(): int
	{
		if ( ! is_null ( $this -> OnAlchemyItemPlaced ) )
		{
			return $this -> OnAlchemyItemPlaced;
		}
		
		$data = $this -> readLastDataLibreAlchemy();
		
		$exception = $this -> exception;
		$this -> exception = false;
		
		if ( ! $this -> isAction( $data, 'OnAlchemyCanceled' ) )
		{
			//$this -> exception = $exception;
			
			throw new AlchemyException( 'Требуется перезагрузка Алхимии.' );
		}
		
		$this -> exception = $exception;
		
		$this -> isAction( $data, 'OnAlchemyItemPlaced' );
		
		if ( is_numeric ( $data['data'] ) && $data['data'] > 0 )
		{
			return $this -> OnAlchemyItemPlaced = ( int ) $data['data'];
		}
		
		throw new AlchemyException( 'Возможно, есть рецепты: Неизвестно.' );
	}
	...
	public function preparePotion( SymfonyStyle $io ): array
	{
		...
		throw new AlchemyReactionEmpty( 'Требуемые по списку зелья отсутствуют.' );
	}
